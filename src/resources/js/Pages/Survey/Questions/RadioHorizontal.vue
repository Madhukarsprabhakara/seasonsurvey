...
<template>
  <div class="mb-6">
    ...
    <!--<p class="text-sm text-gray-500">{{props.qid.question_details.title}}</p>-->
    <fieldset class="mt-2">
      ...
      <div class="space-y-4 ml-2 sm:flex sm:items-center sm:space-x-10 sm:space-y-0">
        <div v-for="option_id in props.qid.question_option_ids" key="option_id.id" class="flex items-center">
          ...
          <label :for="'s_'+option_id.id" class="ml-3 block text-sm font-normal leading-6 text-gray-700">{{ option_id.option_detail.title }}</label>
        </div>
